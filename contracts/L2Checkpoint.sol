...
import "./interfaces/IL2WithdrawableERC721.sol";
...
contract L2Checkpoint is ReentrancyGuard {
    ...
    function _withdrawAndMint(
        IL2WithdrawableERC721 l2Token,
        uint256 tokenID,
        address recipient
    ) public nonReentrant {
        // Take control of their token by transfering it to this contract.
        l2Token.safeTransferFrom(msg.sender, address(this), tokenID);
        ...
        address l1Bank =
            l2Token.approvalWithdraw({id: tokenID, recipient: l1Broker});
        ...
        minterMessenger.sendMessage(
            ...
            abi.encodeWithSignature(
                "mintL1NFT(address,address,address,uint256)",
                recipient,
                l2Token.l1Address(),
                address(l2Token),
                l1Bank,
    ...
    /// @param tokenID The amount (in the decimal scale of the l2/l1 ERC20) to withdraw.
    function withdrawAndMint(IL2WithdrawableERC721 l2Token, uint256 tokenID)
        external
    {
        _withdrawAndMint(l2Token, tokenID, msg.sender);
    }
}
